# 🌍 Master Prompt for Solar PV Plant BOQ
...
Follow IEC 60364-5-54, IEC 62561, IEC 60228, IEC 61869, IEC 60255, IEC 61643-11, IEC 60099-4, IEC 61439, IEC 62271, DLMS/IEC 62056, IS 3043.  
...
(This section shall be replaced by the input parameters that we created)  

- **DC side**: Structure type, module dimensions, table count, layout, rows (for ballasted), string Isc (A), strings per inverter  
- **Lightning protection**: Plant area (m²), soil type  
- **AC side**: System type (LV string / HV central), number of inverters, LV inverter data (V/A, distances, cable sizes, breaker ratings), HV data (inverter qty, IDT & PT details with V/A, cable distances & sizes, transformer MVA & %Z optional), yard size (if not given → LV 30×30 m, HV 40×40 m), target resistance (if not given → LV 5 Ω, HV 1 Ω)  
- **Fixed (do not change)**: Protective device = String fuse, Material = Tinned copper, Insulation = PVC, Rail bonding mode = Bonding clamps, Drop rule = one drop per N tables  
